<div class="container">
    <div class="row bar">
        <div class="col-md-12">
            <p class="text-muted lead text-center">Los destacados de {{ $mes }}</p>
            <div class="products-big">
                <div class="row products">


                    <div class="col-lg-3 col-md-4">
                        <div class="product">
                            <div class="image"><a href="shop-detail.html"><img src="img/product9.jpg" alt="" class="img-fluid image1"></a></div>
                            <div class="text">
                                <h3 class="h5"><a href="shop-detail.html">Blusa Canadá Blanca</a></h3>
                                <p class="price">$146.00</p>
                            </div>
                        </div>
                    </div>

                    <div class="col-lg-3 col-md-4">
                        <div class="product">
                            <div class="image"><a href="shop-detail.html"><img src="img/product7.jpg" alt="" class="img-fluid image1"></a></div>
                            <div class="text">
                                <h3 class="h5"><a href="shop-detail.html">Blusa Tenerife Negra</a></h3>
                                <p class="price">
                                    <del>$280</del> $123.00
                                </p>
                            </div>
                            <div class="ribbon-holder">
                                <div class="ribbon sale">OFERTA</div>
                                <div class="ribbon new">NUEVO</div>
                            </div>
                        </div>
                    </div>

                    <div class="col-lg-3 col-md-4">
                        <div class="product">
                            <div class="image"><a href="shop-detail.html"><img src="img/product6.jpg" alt="" class="img-fluid image1"></a></div>
                            <div class="text">
                                <h3 class="h5"><a href="shop-detail.html">Blusa Alaska Blanca</a></h3>
                                <p class="price">$146.00</p>
                            </div>
                        </div>
                    </div>
                    <div class="col-lg-3 col-md-4">
                        <div class="product">
                            <div class="image"><a href="shop-detail.html"><img src="img/product5.jpg" alt="" class="img-fluid image1"></a></div>
                            <div class="text">
                                <h3 class="h5"><a href="shop-detail.html">Vestido Singapur Plata</a></h3>
                                <p class="price">$175.00</p>
                            </div>
                        </div>
                    </div>
                    <div class="col-lg-3 col-md-4">
                        <div class="product">
                            <div class="image"><a href="shop-detail.html"><img src="img/product8.jpg" alt="" class="img-fluid image1"></a></div>
                            <div class="text">
                                <h3 class="h5"><a href="shop-detail.html">Camisa Rayas Mozambique</a></h3>
                                <p class="price">
                                    <del>$280</del> $134.00
                                </p>
                            </div>
                            <div class="ribbon-holder">
                                <div class="ribbon sale">OFERTA</div>
                                <div class="ribbon new">NUEVO</div>
                            </div>
                        </div>
                    </div>
                    <div class="col-lg-3 col-md-4">
                        <div class="product">
                            <div class="image"><a href="shop-detail.html"><img src="img/product10.jpg" alt="" class="img-fluid image1"></a></div>
                            <div class="text">
                                <h3 class="h5"><a href="shop-detail.html">Camisa Santiago Flores</a></h3>
                                <p class="price">$156.00</p>
                            </div>
                            <div class="ribbon-holder">
                                <div class="ribbon new">NUEVO</div>
                            </div>
                        </div>
                    </div>
                    <div class="col-lg-3 col-md-4">
                        <div class="product">
                            <div class="image"><a href="shop-detail.html"><img src="img/product11.jpg" alt="" class="img-fluid image1"></a></div>
                            <div class="text">
                                <h3 class="h5"><a href="shop-detail.html">Blusa Cuba Roja</a></h3>
                                <p class="price">$113.00</p>
                            </div>
                            <div class="ribbon-holder">
                                <div class="ribbon new">NUEVO</div>
                            </div>
                        </div>
                    </div>
                    <div class="col-lg-3 col-md-4">
                        <div class="product">
                            <div class="image"><a href="shop-detail.html"><img src="img/product12.jpg" alt="" class="img-fluid image1"></a></div>
                            <div class="text">
                                <h3 class="h5"><a href="shop-detail.html">Blusa Maldivas Blanca</a></h3>
                                <p class="price">$120.00</p>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>
